#  AWS Cloud Labs

Welcome to my AWS Lab Experience with Console. 

##  Objective
To translate theoretical knowledge into practical skills by building secure, scalable, and cost-effective cloud solutions.

##  Repository Structure

| Service | Focus Area |
| :--- | :--- |
| **compute/** | EC2 instances, Load Balancers, Auto Scaling Groups |
| **storage/** | S3 Buckets, Lifecycle Policies, EFS |
| **iam/** | User permissions, Roles, JSON Policies |
| **networking/** | VPCs, Subnets, Route Tables, Security Groups |
| **serverless/** | Lambda functions, API Gateway |
| **monitoring/** | CloudWatch Alarms, SNS notifications |

##  Tools & Technologies
* **AWS Console** (Manual Configurations)
* **AWS CLI** (Command Line Interface)
* **Bash Scripting** (User Data & Automation)
* **Diagrams.py / Draw.io** (Architecture Design)

---
*Disclaimer: These labs are for educational purposes and follow AWS Free Tier limits whenever possible.*
